...
|---|---|---|
| React static build | `destino-v.com` (apex) | The public site users see |
| Laravel API + Filament admin | `api.destino-v.com` (subdomain) | The REST API, admin panel, MySQL data, file uploads |
...
1. In hPanel, your primary domain `destino-v.com` is the React site.
2. **Subdomains** → **Create Subdomain**.
   - Subdomain: `api`
   - Document root: leave default (`public_html/api.destino-v.com/`).
3. After creation, the subdomain root maps to a folder under your home directory; note the path (something like `/home/<user>/domains/api.destino-v.com/public_html`).
...
### C. Laravel deploy (`api.destino-v.com`)
...
3. Move the Laravel app into place. Two layouts work; pick one and stick with it:
   ...
   **Option 2 — change the document root in hPanel** to point directly at `domains/api.destino-v.com/destino-backend/backend/public`. Same end result, different button.
...
8. Create the first admin user:
   ...
   ```
   Answer the prompts (name, email, password). This is your admin login for `https://api.destino-v.com/admin`.
...
1. On your local machine:
   ...
   npm run build
   ```
2. The build output is in `dist/`. Upload **the contents** of `dist/` (not the folder itself) to `~/domains/destino-v.com/public_html/`. Use the **File Manager** in hPanel or SFTP.
...
1. `https://api.destino-v.com/api/v1/health` returns `{ "status": "ok", "time": "..." }`.
2. `https://api.destino-v.com/admin` shows a login form. Log in with the credentials from D.8.
3. `https://destino-v.com/` loads, shows featured cars, and the header shows a JST clock.
4. `https://destino-v.com/stock` shows a paginated grid.
5. `https://destino-v.com/contact` — submit the inquiry form. Email lands at `export@destino.jp` within 1–2 minutes. Inquiry appears in the Filament panel under **Inquiries**.
6. `https://destino-v.com/delivered` — submit a review. It appears in **Testimonials** with status **Pending**. Approve it. Refresh `/delivered` and confirm it appears.
7. `https://destino-v.com/shipping` — the embedded PDF loads (or shows the fallback if no PDF has been uploaded yet — upload one via **Page Contents → shipping**).
...
```bash
ssh <user>@<host>
cd ~/domains/api.destino-v.com/destino-backend
git pull
cd backend
composer install --no-dev --optimize-autoloader
...
Test:
- `https://api.destino-v.com/api/v1/health` returns 200.
...
Then upload `dist/*` to `~/domains/destino-v.com/public_html/`, replacing the previous build. Keep the `.htaccess` file in place.

Test:
- Hard-refresh `https://destino-v.com/`. Service workers / browser cache can hide changes; `Ctrl+Shift+R` (Win) or `Cmd+Shift+R` (Mac).
...
```bash
tail -f ~/domains/api.destino-v.com/destino-backend/backend/storage/logs/laravel-$(date +%Y-%m-%d).log
```

Run the sync command manually (mostly to test that creds work):
```bash
cd ~/domains/api.destino-v.com/destino-backend/backend && php artisan stock:sync
```

Verify the cron is firing — should see "Running scheduled command" lines every minute:
```bash
tail -n 20 ~/domains/api.destino-v.com/destino-backend/backend/storage/logs/laravel-$(date +%Y-%m-%d).log
```
